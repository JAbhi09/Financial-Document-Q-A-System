...
#     - total_assets
#     - depreciation
#     - sga (Selling, General, and Admin expenses)
#     - total_debt
#     - net_income
#     - operating_cash_flow
    
#     If any key is missing or 0, it tries to handle safely but may affect accuracy.
#     """
...
#         dsri = safe_div(safe_div(current.get('receivables', 0), current.get('revenue', 1)),
#                         safe_div(prior.get('receivables', 0), prior.get('revenue', 1)))
        
#         # Gross Margin Index (GMI)
#         gmi = safe_div(safe_div(prior.get('revenue', 0) - prior.get('cogs', 0), prior.get('revenue', 1)),
#                        safe_div(current.get('revenue', 0) - current.get('cogs', 0), current.get('revenue', 1)))
        
#         # Asset Quality Index (AQI)
#         aqi = safe_div(1 - safe_div(current.get('current_assets', 0) + current.get('ppe', 0), current.get('total_assets', 1)),
#                        1 - safe_div(prior.get('current_assets', 0) + prior.get('ppe', 0), prior.get('total_assets', 1)))
        
#         # Sales Growth Index (SGI)
#         sgi = safe_div(current.get('revenue', 0), prior.get('revenue', 1))
        
#         # Depreciation Index (DEPI)
#         depi = safe_div(safe_div(prior.get('depreciation', 0), prior.get('depreciation', 0) + prior.get('ppe', 0)),
#                         safe_div(current.get('depreciation', 0), current.get('depreciation', 0) + current.get('ppe', 0)))
        
#         # SG&A Index (SGAI)
#         sgai = safe_div(safe_div(current.get('sga', 0), current.get('revenue', 1)),
#                         safe_div(prior.get('sga', 0), prior.get('revenue', 1)))
        
#         # Leverage Index (LVGI)
#         lvgi = safe_div(safe_div(current.get('total_debt', 0), current.get('total_assets', 1)),
#                         safe_div(prior.get('total_debt', 0), prior.get('total_assets', 1)))
        
#         # Total Accruals to Total Assets (TATA)
#         tata = safe_div(current.get('net_income', 0) - current.get('operating_cash_flow', 0),
#                         current.get('total_assets', 1))
        
#         # M-Score calculation
#         # Constants from Beneish (1999)
...
#         return {"error": str(e), "m_score": -999}
...
#     Expected keys in input dicts:
#     - receivables, revenue, cogs, current_assets, ppe, total_assets
#     - depreciation, sga, total_debt, net_income, operating_cash_flow
#     """
...
#         # Depreciation Index (DEPI)
#         depi = safe_div(safe_div(prior.get('depreciation', 0), prior.get('depreciation', 0) + prior.get('ppe', 1)),
#                         safe_div(current.get('depreciation', 0), current.get('depreciation', 0) + current.get('ppe', 1)))
#         if depi == 0:
#             depi = 1.0
...
#                   0.115*depi - 0.172*sgai + 4.679*tata - 0.327*lvgi
    ...
    #     }
    # except Exception as e:
    #     return {"error": str(e), "m_score": -999, "data_source": "error"}
